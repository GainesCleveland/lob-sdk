...
export class CircleShape implements CollisionShape {
  ...
  /**
   * Overlap with another circle as a fraction (0..1) of the smaller disc's area
   * (matching convexOverlapRatio's semantics): 0 when disjoint, 1 when the smaller
   * sits fully inside the larger, the lens-area fraction in between.
   */
  private overlapWithCircle(other: CircleShape): number {
    ...
    return minArea > 0 ? Math.min(lens / minArea, 1) : 0;
  }
